...
var callWrapperTemplate = babel.template('$runtime$.$method$($file$, $line$, $object$, $property$, $index1$, $index2$, $returnArray$)');
...
function streamlineCall(t, scope, state, node, method, index1, index2, returnArray) {
	...
	if (t.isMemberExpression(callee)) {
		...
		if (binding && binding.identifier.isStreamlineId && method === 'await' && index2 == null && !returnArray) {
			return t.callExpression(t.memberExpression(callee, t.stringLiteral('awaitWrapper-' + index1), true), node.arguments);
		}
		*/
	...
	var fn = callWrapperTemplate({
		$runtime$: t.identifier(state.streamline.runtimeVar),
		$method$: t.identifier(method),
		$file$: t.identifier(state.streamline.filenameVar),
		$line$: t.numericLiteral(node.loc ? node.loc.start.line : 0),
		$object$: object,
		$property$: property,
		$index1$: t.numericLiteral(index1 || 0),
		$index2$: index2 != null ? t.numericLiteral(index2) : t.nullLiteral(),
		$returnArray$: t.booleanLiteral(returnArray),
	}).expression;
		...
		visitor: {
			...
			Program: {
				...
				exit: function(path, state) {
					...
					node = t.program(runtimePrologue(t, state).concat(state.streamline.programIsAsync ? asyncProgramWrapper(t, state, node, scope) : node.body));
