...
function futurify(t, node, index) {
	return asyncify(t, node, 'futurify', index);
}
...
function promisifyNew(t, node, index) {
	node.arguments.splice(index, 1)[0];
	return t.callExpression(
			...
			NewExpression: {
				enter: function(node, parent, scope, file) {
					...
					if ((index = _.findIndex(node.arguments, is_)) >= 0) {
						return t.awaitExpression(promisifyNew(t, node, index));
					}
				}
			}
		}
