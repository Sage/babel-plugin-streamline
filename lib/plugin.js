...
makeTemplate('function-wrapper', '$runtime$.async(function $id_$($params$) { $body$ }, $index$)');
...
function streamlineFunction(t, state, node, scope, index) {
	...
	var innerCall = applyTemplate('function-wrapper', {
		$runtime$: t.identifier(state.streamline.runtimeVar),
		$id_$: t.identifier(scope.generateUid(node.id ? node.id.name : guessName(t, this.parent))),
		$params$: params.slice(),
		$body$: node.body,
...
function runtimePrologue(t, state) {
	var prologue = [];
	if (state.streamline.runtime.async || state.streamline.runtime.generator || state.streamline.needsRegenerator) {
...
function hoist(t, scope, nodes) {
	...
	var decl = t.variableDeclaration("var", nodes.filter(function(node) {
		...
		var hoistName = names[node.id.name] = scope.generateUid(node.id.name);
		return t.variableDeclarator(t.identifier(node.id.name),
	...
	return [decl].concat(nodes.map(function(node) {
		if (t.isFunctionDeclaration(node)) {
			...
			node.id = scope.generateUidIdentifier(node.id.name);
		...
		visitor: {
			...
			Program: {
				enter: function(node, parent, scope, state) {
					...
					return t.program(hoist(t, scope, node.body));
				},
				exit: function(node, parent, scope, state) {
					if (!canTransform(state) && !state.streamline.needsRegenerator) return;
			...
			Function: function(node, parent, scope, state) {
				...
				if (node.async || node.generator) state.streamline.needsRegenerator = true;
				if (!canTransform(state)) return;
			...
			CallExpression: function(node, parent, scope, state) {
				...
				if ((index1 = findIndex(node.arguments, isCallbackArg)) >= 0) {
					...
					if (t.isProgram(funcScope.block)) {
			...
			"YieldExpression|AwaitExpression": function(node, parent, scope, state) {
				// regenerator transform does not automatically add its variable to we do it (even on .js files)
				state.streamline.needsRegenerator = true;
			},
