...
function isFutureArg(node) {
	return node.type === 'UnaryExpression' && (node.operator === '!' || node.operator === 'void') && is_(node.argument);
}
		...
		visitor: {
			...
			CallExpression: function(node, parent, scope, state) {
				...
				if (funcScope.block.isStreamline) {
					// not a call with _ but we are inside an async function and we need to deal with CoffeeScript IIFE 
					var innerFunc;
