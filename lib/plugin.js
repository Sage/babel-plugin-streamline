...
function awaitCall(t, state, node, index, returnArray) {
	return awaitWrap(t, state, streamlineCall(t, state, node, 'await', index, null, returnArray));
}

function awaitCall2(t, state, node, index1, index2) {
	return awaitWrap(t, state, streamlineCall(t, state, node, 'await', index1, index2, null));
}
		...
		visitor: {
			...
			Function: function(node, parent, scope, state) {
				...
				if ((index = findIndex(node.params, is_)) >= 0) {
					...
					param.$done = true;
					if (node.generator) 
						throw state.errorWithNode(param, "parameter _ not allowed in generator function");
					if (node.async) 
						throw state.errorWithNode(param, "parameter _ not allowed: function already marked `async`");
			...
			CallExpression: function(node, parent, scope, state) {
				...
				if ((index = findIndex(node.arguments, isCallbackArg)) >= 0) {
					if (!funcScope.parent) {
						...
						if (!funcScope.block.isStreamline) 
							throw state.errorWithNode(node, "unexpected `_` argument: enclosing function does not have an `_` parameter.");
					}
					node.arguments[index] = t.literal(true);
					var index2 = findIndex(node.arguments, is_);
					if (index2 >= 0) {
						node.arguments[index2] = t.literal(true);
						if (findIndex(node.arguments, is_) >= 0) 
							throw state.errorWithNode(node, "async call cannot have more than 2 _ arguments");
						return awaitCall2(t, state, node, index, index2);
					}
				...
				if ((index = findIndex(node.arguments, isPromiseArg)) >= 0) {
					node.arguments[index] = t.literal(false);
					return t.memberExpression(future(t, state, node, index), t.identifier('promise'));
				}
				...
				if ((index = findIndex(node.arguments, isArray_)) >= 0) {
					node.arguments[index] = t.literal(true);
					return awaitCall(t, state, node, index, true);
				}
				...
				if (funcScope.block.isStreamline) {
					...
					if (t.isFunctionExpression(callee) && callee.params.length === 0 && node.arguments.length === 0) {
						...
						innerFunc = callee;
					} else if (t.isMemberExpression(callee) && t.isFunctionExpression(callee.object) &&
						callee.object.params.length === 0) {
						if (callee.property.name === 'call' && node.arguments.length === 1 && 
							t.isThisExpression(node.arguments[0])) {
							 // (function() { ... }).call(this) ->  (await (async function(_) { ... }).call(this, _)
							innerFunc = callee.object;
						} else if (callee.property.name === 'apply' && node.arguments.length === 2 && 
							t.isThisExpression(node.arguments[0]) && node.arguments[1].name === 'arguments') {
