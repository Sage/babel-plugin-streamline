...
function isTrapArg(node) {
	return node.type === 'UnaryExpression' && node.operator === '!' && isFutureArg(node.argument);
}
...
makeTemplate("trap", "(function(err) { if (err) throw err; })");
makeTemplate("program-wrapper", "$runtime$.async(function() { $body$ }, 0)($trap$)");

function asyncProgramWrapper(t, state, node) {
	var wrapper = applyTemplate("program-wrapper", {
		$runtime$: t.identifier(state.streamline.runtimeVar),
		$body$: node.body,
		$trap$: applyTemplate("trap", {}),
	});
	...
	return new Plugin("streamline", {
		visitor: {
			...
			CallExpression: function(node, parent, scope, state) {
				...
				if ((index1 = findIndex(node.arguments, isTrapArg)) >= 0) {
					node.arguments[index1] = applyTemplate('trap', {});
					return node;
				}
