...
function runtimePrologue(t, state) {
	...
	if (canTransform(state)) {
		...
		if (!/\/streamline-runtime\/lib\/builtins/.test(state.opts.filename)) {
			prologue.push(t.expressionStatement(t.callExpression(t.identifier('require'), [t.literal("streamline-runtime/lib/builtins")])));
		}
	}
	return prologue;
}
...
function canTransform(state) {
	return /^(unknown|.*\._js)$/.test(state.opts.filename);
}
