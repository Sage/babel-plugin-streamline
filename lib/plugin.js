...
function awaitWrap(t, state, expression) {
	const rt = state.streamline.runtime;
...
function awaitCall(t, scope, state, node, index1, index2, returnArray) {
	function isLocalStreamline(node) {
		...
		const n = binding.path.node.init;
	...
	if (state.streamline.runtime.name === "fibers" && index2 == null && !returnArray) {
		...
		if (!state.streamline.fileOptions['protect-builtins'] && isArrayBuiltin(node)) {
			const name = node.callee.property.name.replace(/_$/, '');
...
const functionWrapperTemplate = babel.template('$runtime$.async(function $id_$($params$) { $body$ }, $index$, $arity$)');
...
function streamlineFunction(t, state, node, scope, index) {
	...
	const body = node.type === 'ArrowFunctionExpression' && !t.isStatement(node.body) ? t.returnStatement(node.body) : node.body;
	const innerCall = functionWrapperTemplate({
	...
	const innerFunc = innerCall.arguments[0];
...
const requireVarTemplate = babel.template("var $name = typeof require === 'function' ? require($path) : Streamline.require($path);");
...
const programWrapperTemplate = babel.template("$runtime$.async($arg$, 0, 1).call(this, function(err) { if (err) throw err; })");

function asyncProgramWrapper(t, state, node, scope) {
	const wrapper = t.functionExpression(t.identifier(scope.generateUid('')), [], //
...
function hoist(t, scope, nodes) {
	...
	const other = nodes.filter(function(node) {
		return !t.isFunctionDeclaration(node);
	});
...
function checkProgramAsync(t, node, parent, scope, state) {
	function traverseCall(path, state) {
		...
		const index = findIndex(node.arguments, isCallbackArg);
		if (index >= 0) {
			...
			const innerFunc = unwrapIIFE(t, node);
		...
		visitor: {
			...
			Program: {
				enter(path, state) {
					...
					const n = t.program(hoist(t, scope, node.body));
					n._entered = true;
					n.isStreamline = checkProgramAsync(t, n, parent, scope, state);
					path.replaceWith(n);
				},
				exit(path, state) {
					const node = path.node;
					...
					const n = t.program(runtimePrologue(t, state).concat(state.streamline.programIsAsync ? asyncProgramWrapper(t, state, node, scope) : node.body));
			...
			CallExpression(path, state) {
				...
				if ((index1 = findIndex(node.arguments, isCallbackArg)) >= 0) {
					...
					const index2 = findIndex(node.arguments, is_);
					if (index2 >= 0) {
			...
			"YieldExpression|AwaitExpression"(path, state) {
				// regenerator transform does not automatically add its variable to we do it (even on .js files)
			...
			BlockStatement(path, state) {
				...
				const scope = path.scope;
				if (node._entered) return;
				const n = t.blockStatement(hoist(t, scope, node.body));
				n._entered = true;
				path.replaceWith(n);
			},
		}
